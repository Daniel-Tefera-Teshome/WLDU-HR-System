...
<script type="text/javascript" language="javascript" >
 $(document).ready(function(){
  ...
  $(document).on('click', '#insert', function(){
   var region = $('#data1').text();
   var city = $('#data2').text();
   var zone = $('#data3').text();
   var wereda = $('#data4').text();
   var kebele = $('#data5').text();
   var house_no = $('#data6').text();
   var phone_no = $('#data7').text();
   var emp_id = $('#data8').text();
   	...
   	)
   {
    $.ajax({
     url:"insertr.php",
     method:"POST",
     data:{region:region, city:city, zone:zone, wereda:wereda, kebele:kebele, house_no:house_no, phone_no:phone_no, emp_id:emp_id},
     success:function(data)
     {
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
      $('#user_data').DataTable().destroy();
