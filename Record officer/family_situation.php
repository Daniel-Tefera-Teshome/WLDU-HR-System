...
<script type="text/javascript" language="javascript" >
 $(document).ready(function(){
  ...
  $(document).on('click', '#insert', function(){
   var martial_status = $('#data1').text();
   var husbund_or_wife_name = $('#data2').text();
   var emp_id = $('#data3').text();

   if(
	   	martial_status != '' &&
	   	husbund_or_wife_name != '' && 
	   	emp_id != ''
   	)
   {
    $.ajax({
     url:"insertf.php",
     method:"POST",
     data:{martial_status:martial_status, husbund_or_wife_name:husbund_or_wife_name, emp_id:emp_id},
     success:function(data)
     {
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
      $('#user_data').DataTable().destroy();
